...
import { AccessControl, Company, Device, Site } from '@/common/models';
...
const columns = (
...
) => [
  {
    ...
    render: ({ data }: CellRender<Company>) => (
      <Flex
  ...
  },
  {
    ...
    render: ({ data }: CellRender<Company>) => data.role?.name ?? '-',
  },
  {
    name: 'address',
    defaultFlex: 1,
  ...
  },
  {
    ...
    render: ({ data }: CellRender<Company>) => <Status status={data.status} />,
  },
  {
    name: 'phone',
    defaultFlex: 1,
