...
const columns = (
...
) => [
  {
    ...
    render: ({ data }: CellRender<Resident>) => (
      <Flex
      ...
      >
        ...
        />
        <Text>{common.getUserFullName(data as User)}</Text>
      </Flex>
    ),
  },
  {
    ...
    render: ({ data }: CellRender<Resident>) => data.role?.name ?? '-',
  },
  {
    name: 'address',
    defaultFlex: 1,
  ...
  },
  {
    name: 'apartment',
    defaultFlex: 1,
    header: t('tables.users.cells.apartment'),
    render: ({ data }: CellRender<Resident>) => data.apartment?.number ?? '-',
  },
  {
    name: 'company',
    defaultFlex: 1,
    header: t('tables.users.cells.company'),
    render: ({ data }: CellRender<Resident>) => data.company?.name ?? '-',
  },
  {
    name: 'floor',
    defaultFlex: 1,
    ...
    render: ({ data }: CellRender<Resident>) => {
      if (data.type === UserType.RESIDENT) {
        return data.companyId ? data.company?.floor : data.apartment?.floor;
      }
  ...
  },
  {
    ...
    render: ({ data }: CellRender<Resident>) => <Status status={data.status} />,
  },
  {
    name: 'phone',
    defaultFlex: 1,
